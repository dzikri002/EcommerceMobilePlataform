...
import CollectionThumbFour from "../productCollection/CollectionThumbFour";
...
class CollectionTH4 extends Component {
    ...
    render() {
        return <Container>
            <Header navigation={this.props.navigation} title="Collection4 Lista" />
            <ScrollView>
                {this._renderCollectionList(homeData.four)}
            ...
            <ListPanel onPressSeeAll={() => this._pressSeeAllProducts({ navBarTitle: data.title })} title={data.title} description={data.description}>

                {
                    data.items.map((item, idx) => {
                        return <CollectionThumbFour onPress={() => this._pressProduct(item.id)} key={idx} {...item} />
                    })
                }
